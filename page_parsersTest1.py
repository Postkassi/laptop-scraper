...
def advania_parser(url, retailer, baseurl, items):
    ...
    for product in products:
        ...
        standard_price = price_info.get('price')  # Get the first "price" value
        special_price = discount_price_info.get('price')
...
#     if script_tag:
#         # Extract the JSON data from the script tag
#         json_text = re.search(r'items: (\[.*?\])', script_tag.string, re.DOTALL).group(1)
#         products = json.loads(json_text)
